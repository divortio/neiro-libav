...
Audio processing for JavaScript. Chainable, immutable, serverless-ready.

neiro (音色, "tone color") is a JavaScript library for processing audio on the server. Loudness normalization, true peak limiting, silence trimming, fades, slicing, and more — all through a clean, chainable API.
...
```javascript
import { AudioTrack } from "@divortio/neiro-libav";
...
writeFileSync("output.mp3", await result.toMp3());
```
...
writeFileSync("output.wav", await track.toWav());
```
...
neiro gives you a single, typed API that handles the common cases. The DSP internals (ITU-R BS.1770-4 loudness, true peak detection via 4x oversampling, K-weighting filters) are implemented in pure JavaScript — no WASM, no native binaries, no ffmpeg.

Two small runtime dependencies handle MP3 codec work: `lamejs` for encoding and `audio-decode` for decoding. Everything else is hand-written pure JavaScript.
...
// Export
await track.toMp3({ bitrate: 128 }); // Buffer
await track.toWav(); // Buffer
...
const output = track
  ...
  .fadeIn({ ms: 500 })
  .fadeOut({ ms: 2000 });
const outputBuffer = await output.toMp3({ bitrate: 192 });
```
...
const processed = sfx
  ...
  .trimSilence()
  .fadeOut({ ms: 10 });
const output = await processed.toMp3();
```
...
const processed = music
  ...
  .fadeIn({ ms: 500 })
  .fadeOut({ ms: 2000 });
const output = await processed.toMp3({ bitrate: 192 });
```
...
const sequence = beep
  ...
  .concat({ other: gap })
  .concat({ other: beep });
const output = await sequence.toWav();
```
